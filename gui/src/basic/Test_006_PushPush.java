...
class PushNemo{
	private int x, y, w, h;
	private Color c;
	public PushNemo(int x, int y, int w, int h) {
		this.x = x;
		this.y = y;
		this.w = w;
		this.h = h;
...
class PushPanel extends JPanel implements MouseListener, Runnable{
	...
	private PushNemo stop;
	private JButton[] arrow = new JButton[4];
	private boolean [] arrowChk = new boolean [4];
	...
	private void setNemo() {
		...
		this.stop = new PushNemo(rX, rY, 100, 100);
		...
		while(true) {
			...
			if(check) {
				this.move = new PushNemo(rX, rY, 100, 100);
				break;
			}
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		if(e.getSource() instanceof JButton) {
			...
			if(target == this.arrow[this.UP]) {
				this.arrowChk[this.UP] = true;
			}
			else if(target == this.arrow[this.LEFT]) {
				this.arrowChk[this.LEFT] = true;
			}
			else if(target == this.arrow[this.DOWN]) {
				this.arrowChk[this.DOWN] = true;
			}
			else if(target == this.arrow[this.RIGHT]) {
				this.arrowChk[this.RIGHT] = true;
			}
		}
	}
	@Override
	public void mouseReleased(MouseEvent e) {
		if(e.getSource() instanceof JButton) {
			JButton target = (JButton) e.getSource();
			if(target == this.arrow[this.UP]) {
				this.arrowChk[this.UP] = false;
			}
			else if(target == this.arrow[this.LEFT]) {
				this.arrowChk[this.LEFT] = false;
			}
			else if(target == this.arrow[this.DOWN]) {
				this.arrowChk[this.DOWN] = false;
			}
			else if(target == this.arrow[this.RIGHT]) {
				this.arrowChk[this.RIGHT] = false;
			}
		}
	}
	@Override
	...
	public void run() {
		while(true) {
			if(this.arrowChk[this.UP]) {
				up();
			}
			else if (this.arrowChk[this.LEFT]) {
				left();
			}
			else if (this.arrowChk[this.DOWN]) {
				down();
			}
			else if (this.arrowChk[this.RIGHT]) {
				right();
			}
